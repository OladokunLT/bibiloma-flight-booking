document.addEventListener("DOMContentLoaded", async () => {
  ...
  try {
    const countResponse = await fetch(`${BASE_API_URL}/api/flight/contact-messages/count/`, {
      headers: {
    ...
    const messagesResponse = await fetch(`${BASE_API_URL}/api/flight/contact-messages/`, {
      headers: {
        Authorization: `Bearer ${accessToken}`,
      },
    });
  ...
  contactFilterButton.addEventListener("click", () => {
    ...
    const filteredMessages = messages.filter(({ full_name, email, message }) => {
      return (
...
async function deleteMessage(id) {
  ...
  try {
    const response = await fetch(`${BASE_API_URL}/api/flight/contact-messages/${id}/`, {
      method: "DELETE",
      headers: {
        Authorization: `Bearer ${accessToken}`,
      },
